...
const App: React.FC = () => {
  ...
  const filteredScores = scores.filter(
    (score) => score.date === selectedDate.toISOString().split("T")[0]
  );
  // Sort by attempts in ascending order
  filteredScores.sort(
    (a, b) => a.attempts - b.attempts || a.name.localeCompare(b.name)
  );
  ...
  const MainContent: React.FC<{
    onSubmitScore: (newScore: Score) => Promise<void>;
    rankedScores: Score[];
  }> = ({ onSubmitScore, rankedScores }) => {
    const location = useLocation();

    return (
      <Stack spacing={5}>
        <Navbar onDateChange={setSelectedDate} />
        {location.pathname !== "/this-week" && (
          <ScoreForm onSubmitScore={onSubmitScore} />
        )}
